...
TEST(ServerException, IllegalFunction)
{
...
TEST(ReadBits, Valid_Frames)
{
    ...
    //read status of 4 coils starting from address 2
    //there are 10 coils so reading 2-5 is okay
    s.write({1, 1, 0, 2, 0, 4});
    while (!s.available())
        ;
...
TEST(ReadRegisters, Valid_Frames)
{
...
TEST(ReadRegisters, Invalid_Address)
{
    ...
    while (!s.available())
        ;
    ASSERT_EQ(std::vector<uint8_t>({1, 3 + 0x80, 2}), s.readBytes());
    ...
    while (!s.available())
        ;
    ASSERT_EQ(std::vector<uint8_t>({1, 4 + 0x80, 2}), s.readBytes());
...
TEST(ReadRegisters, Invalid_Data)
{
    ...
    while (!s.available())
        ;
    ASSERT_EQ(std::vector<uint8_t>({1, 3 + 0x80, 3}), s.readBytes());
    ...
    while (!s.available())
        ;
    ASSERT_EQ(std::vector<uint8_t>({1, 4 + 0x80, 3}), s.readBytes());
}

TEST(WriteBits, Force_Single_Coil_Valid_Frames)
{
    //force single coil
    //on address 0
...
TEST(WriteBits, Force_Single_Coil_Invalid_Address)
{
    //force single coil
    //on address 10
    //max address is 9
...
TEST(WriteBits, Force_Single_Coil_Invalid_Data)
{
    //clear single coil
    //on address 0
    s.write({1, 5, 0, 0, 0, 0});
...
TEST(WriteBits, Force_Multiple_Coils_Valid_Frames)
{
    ...
    //coils 2 - 7 should remain untouched
...
TEST(WriteBits, Force_Multiple_Coils_Invalid_Address)
{
    //clear 2 coils
    //from address 8
...
TEST(WriteBits, Force_Multiple_Coils_Invalid_Data)
{
    //force 2 coils
    //from address 8
    s.write({1, 0x0f, 0, 8, 0, 2, 1, 3});
...
TEST(WriteRegisters, Preset_Single_Register_Valid)
{
...
TEST(WriteRegisters, Preset_Single_Register_Invalid_Address)
{
    //preset single register
    //on address 20
    //max address is 19
...
TEST(WriteRegisters, Preset_Multiple_Registers_Valid)
{
    ...
    //check whether holding registers were preset
    s.write({1, 3, 0, 0, 0, 3});
    while (!s.available())
        ;
...
TEST(WriteRegisters, Preset_Multiple_Registers_Invalid_Address)
{
    //preset single register
    //on address 19
...
TEST(WriteRegisters, Preset_Multiple_Registers_Invalid_Data)
{
    //preset single register
    //on address 19
    s.write({1, 6, 0, 19, 8, 11});
